...
public class RuntimeArtifact
{
    ...
    public void Upgrade()
    {
        ...
        level++;
        mainStat.value = CalculateMainStatValue(level);
        ...
        if (level % 10 == 0 && subStats.Count > 0)
        {
            ...
            randomSub.value += increase;
        }
    }
    private float CalculateMainStatValue(int currentLevel)
    {
        float levelCurve = Mathf.Pow(currentLevel, 1.2f);
